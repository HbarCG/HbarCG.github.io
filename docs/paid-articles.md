...
- マスター鍵をなくすと購入者も読めなくなる。`paid-src/` は非公開リポジトリ `HbarCG/hbarcg-paid-src` で管理している
  （`paid-src/` の中がそのリポジトリ。別のパソコンでは `git clone https://github.com/HbarCG/hbarcg-paid-src.git paid-src` で取り出す）
...
8. `paid-src/` の中で原稿をコミットし、非公開リポジトリに push してバックアップする
